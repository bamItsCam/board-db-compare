...
class Command(BaseCommand):
	...
	def populate_the_db(self):
		...
		attributesList = ['Name','Display interfaces', 'RAM', 'SD card', 'Audio output', 'Audio input', 'GPU', 'Type', 'Composite (CVBS)', 'Price', 'USB OTG port', 'CPU', 'Windows supported', 'Operating temperature', 'Real time clock', 'Other interfaces', 'Manufacturer', 'Internal storage', 'Other I/O', 'SoC', 'HDMI', 'VGA', 'IR receiver', 'Wake-on-LAN', 'USB host', 'Ethernet', 'Wi-Fi', 'USB OTG', 'Amperage', 'Rating', 'Android supported', 'LAN', 'Weight', 'ADC support', 'Bluetooth', 'SATA', 'Voltage', 'Model code', 'Board name', 'GPIO pins', 'Size']
		...
		for i in range(0,DB.HIGHEST_KNOWN_PRODUCT_NUMBER):
			...
			#If not invalid, first pad the current attribute dictionary with ?'s for attributes with no data available, then create a new record in the database
				...
				dbBoards.objects.create(name='%s' % attributeDict[attributesList[0]],display='%s' % attributeDict[attributesList[1]],ram='%s' % attributeDict[attributesList[2]],sd='%s' % attributeDict[attributesList[3]],audio_out='%s' % attributeDict[attributesList[4]],audio_in='%s' % attributeDict[attributesList[5]],gpu='%s' % attributeDict[attributesList[6]],board_type='%s' % attributeDict[attributesList[7]],cvbs='%s' % attributeDict[attributesList[8]],price='%s' % attributeDict[attributesList[9]],usb_otg_port='%s' % attributeDict[attributesList[10]],cpu='%s' % attributeDict[attributesList[11]],windows='%s' % attributeDict[attributesList[12]],temp='%s' % attributeDict[attributesList[13]],clock='%s' % attributeDict[attributesList[14]],interfaces='%s' % attributeDict[attributesList[15]],manufacturer='%s' % attributeDict[attributesList[16]],storage='%s' % attributeDict[attributesList[17]],other='%s' % attributeDict[attributesList[18]],soc='%s' % attributeDict[attributesList[19]],hdmi='%s' % attributeDict[attributesList[20]],vga='%s' % attributeDict[attributesList[21]],ir='%s' % attributeDict[attributesList[22]],wake_on_lan='%s' % attributeDict[attributesList[23]],usb='%s' % attributeDict[attributesList[24]],ethernet='%s' % attributeDict[attributesList[25]],wifi='%s' % attributeDict[attributesList[26]],usb_otg='%s' % attributeDict[attributesList[27]],amps='%s' % attributeDict[attributesList[28]],rating='%s' % attributeDict[attributesList[29]],android='%s' % attributeDict[attributesList[30]],lan='%s' % attributeDict[attributesList[31]],weight='%s' % attributeDict[attributesList[32]],adc='%s' % attributeDict[attributesList[33]],bluetooth='%s' % attributeDict[attributesList[34]],sata='%s' % attributeDict[attributesList[35]],volts='%s' % attributeDict[attributesList[36]],model_code='%s' % attributeDict[attributesList[37]],board_name='%s' % attributeDict[attributesList[38]],gpio='%s' % attributeDict[attributesList[39]],size='%s' % attributeDict[attributesList[40]])
